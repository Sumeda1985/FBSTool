feed <- function(input, output, session) {
  ...
  observeEvent(input$feed_ratio_cell_edit, {
    ...
    replaceData(proxy_feed_ratio, df_feed_ratio$data_feed_ratio,
  ...
  observeEvent(input$FeedRatioInsert, {
    ...
    if (length(s) == 0) {
      ...
      feedRatiolistTool <- subset(feedRatiolistTool,
                                  !CPCCode %in% unique(isolate(df_feed_ratio$data_feed_ratio$CPCCode)))
      ...
      ff <- melt.data.table(yy[, c("CPCCode", "Commodity")],
  ...
  observeEvent(input$fileFeedRatiodenormalized, {
    ...
    data_denormalized[, c("ElementCode", "CPCCode") := lapply(.SD, as.character),
    ...
    feed_ratio_Data[, c("ElementCode", "CPCCode") := lapply(.SD, as.character),
                    .SDcols = c("ElementCode", "CPCCode")]
    ...
    feed_ratio_Data <- merge(feed_ratio_Data, all_cpc, by = "CPCCode",
                             all.x = T)
      ...
      sidebarLayout(
        ...
        sidebarPanel(
          ...
          selectizeInput("cpcFeedratio", "CPC Code",
                         selected = NULL,
                         choices = c("",
                                     colnames(df_feed_ratioCountry$data_feed_ratioCountry)
                                     ), multiple = F
                         ),
  ...
  observe({
      updateSelectInput(session, "cpcFeedratio",
                        choices = c("", colnames(df_feed_ratioCountry$data_feed_ratioCountry)))
      updateSelectInput(session, "elementFeedratio",
                        choices = c("", colnames(df_feed_ratioCountry$data_feed_ratioCountry)))
      updateSelectInput(session, "yearFeedratio",
                        choices = c("", colnames(df_feed_ratioCountry$data_feed_ratioCountry)))
      updateSelectInput(session, "valueFeedratio",
                        choices = c("", colnames(df_feed_ratioCountry$data_feed_ratioCountry)))
      updateSelectInput(session, "flagFeedratio",
                        choices = c("", colnames(df_feed_ratioCountry$data_feed_ratioCountry)))
  })
  ...
  observeEvent(input$uploadFeedratio, {
    ...
    if (input$cpcFeedratio == "" | input$elementFeedratio == "" | input$yearFeedratio == "" | input$valueFeedratio == "" | input$flagFeedratio == "") {
      ...
      df_feed_ratio$data_feed_ratio <- df_feed_ratio$data_feed_ratio
        ...
        data <- data[, c(input$cpcFeedratio, input$elementFeedratio,
                         input$yearFeedratio, input$valueFeedratio,
                         input$flagFeedratio),
                     with = F]
      ...
      if (length(names(data)[duplicated(names(data))]) > 0) {
        ...
        setnames(
            data, c(input$cpcFeedratio, input$elementFeedratio,
                    input$yearFeedratio, input$valueFeedratio,
                    input$flagFeedratio),
  ...
  output$feed_ratio <-
    renderDataTable({
      ...
      datatable(df_feed_ratio$data_feed_ratio,
        ...
        options = list(
          ...
          columnDefs = list(
            list(width = "", targets = c(6))
            # list(visible = FALSE, targets = (ncol(df_feed_ratio$data_feed_ratio)-1))
          )
        )
      ) %>%
          formatCurrency(columns = as.character(c(2010:input$endyear)),
  ...
  observeEvent(input$startContinue, {
    ...
    setDT(feedData)
    feedData <- feedData[!duplicated(feedData,
                                     by = c("CPCCode", "Commodity",
                                            "ElementCode", "Element", "Year"))]
    ...
    if (END_YEAR > maxyear + 1) {
      ...
      if (length(yearsToFill) > 0) {
        ...
        sendSweetAlert(
          session = session,
          title = "Error!!",
          text = paste("Please compile Feed data for the year(s) ",
      ...
      feedData[, hidden := ifelse(CPCCode != shift(CPCCode, type = "lead"),
                                  1, 0)]
  ...
  observeEvent(input$feed_values_cell_edit, {
    ...
    replaceData(proxy_feed, df_feed$data_feed, resetPaging = FALSE,
  ...
  observeEvent(input$feedInsert, {
    ...
    if (length(s) == 0) {
      ...
      oo <- merge(ff, all_elements, by.x = "ElementCode", by.y = "ElementCode",
                  all.x = T)
      setcolorder(oo, c("CPCCode", "Commodity", "ElementCode", "Element"))
      oo <- oo[order(CPCCode)]
  ...
  observeEvent(input$delete_btn_feed, {
    ...
    database <- database[!(CPCCode %in% cpc_code_to_remove &
                           ElementCode %in% ele_code_to_remove)]
  ...
  observeEvent(input$fileFeeddenormalized, {
    ...
    feed_Data[, c("ElementCode", "CPCCode") := lapply(.SD, as.character),
              .SDcols = c("ElementCode", "CPCCode")]
  ...
  output$feedCountry <- renderDataTable({
    ...
    datatable(df_feedCountry$data_feedCountry, list(lengthMenu = c(5, 30, 50),
                                                    pageLength = 5))
  })
  ...
  observeEvent(input$uploadFeed, {
      ...
      if (input$cpcFeed == "" | input$elementFeed == "" | input$yearFeed == "" |
          input$valueFeed == "" | input$flagFeed == "") {
          sendSweetAlert(
              session = session,
              title = "Warning !!",
              text = "Invalid data",
          ...
          data <- data[, c(input$cpcFeed, input$elementFeed, input$yearFeed,
                           input$valueFeed, input$flagFeed), with = F]
          if (length(names(data)[duplicated(names(data))]) > 0) {
              sendSweetAlert(
                  session = session,
                  title = "WARNING !!",
              ...
              setnames(
                  data, c(input$cpcFeed, input$elementFeed, input$yearFeed,
                          input$valueFeed, input$flagFeed),
                  c("CPCCode", "ElementCode", "Year", "Value", "Flag")
              )
              ...
              feed_values <- merge(feed_values, all_elements,
                                   by = "ElementCode",
                                   all.x = T)
              feed_values <- merge(feed_values, all_cpc, by = "CPCCode",
                                   all.x = T)
              feed_values <- feed_values[!is.na(Element)]
              feed_values <- wide_format(feed_values)
              feed_values[, hidden := ifelse(CPCCode != shift(CPCCode,
                                                              type = "lead"),
                                             1, 0)]
  ...
  observeEvent(input$saveFeed, {
    ...
    save_to_database(data = data_to_save, countryData,
  ...
  output$feed_values <-
    renderDataTable(
      if (!is.null(df_feed$data_feed)) {
        datatable(df_feed$data_feed,
          rownames = FALSE, class = "cell-border stripe",
          editable = list(target = "cell",
                          disable = list(columns = c(0:as.numeric(which(colnames(df_feed$data_feed) == input$fromyear) - 2)))),
          extensions = c("FixedColumns", "FixedHeader", "Buttons"),
          options = list(
            pageLength = 25,
            dom = "Blfrtip", buttons = I("colvis"),
          ...
          formatStyle(0:ncol(df_feed$data_feed),
            valueColumns = "hidden",
